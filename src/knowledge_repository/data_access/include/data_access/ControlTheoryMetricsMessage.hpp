...
class ControlTheoryMetricsMessage {
  	...
  	public:
        ControlTheoryMetricsMessage(const std::string &name, const int64_t &timestamp, const int64_t &logical_clock, const std::string &source, const std::string &target, const std::string &enactor_kp, const std::string &enactor_ki, const std::string &stability,const std::string &convergence_point, const std::string &settling_time, const std::string &overshoot, const std::string &steady_state_error) : name(name), timestamp(timestamp), logical_clock(logical_clock), source(source), target(target), enactor_kp(enactor_kp), enactor_ki(enactor_ki), stability(stability), convergence_point(convergence_point), settling_time(settling_time), overshoot(overshoot), steady_state_error(steady_state_error){};
        ...
        std::string getEnactorKi() const {return this->enactor_ki;};
